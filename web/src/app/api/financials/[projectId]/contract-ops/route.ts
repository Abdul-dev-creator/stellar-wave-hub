...
export async function GET(
  ...
  try {
    const operations = await getContractInvocations(project.stellar_account_id);
    return Response.json({ operations });
  } catch (err) {
    console.error("Contract ops error:", err);
    return Response.json({ error: "Failed to fetch contract operations from Stellar" }, { status: 502 });
  }
}
